...
class DistressMessageController {
    ...
    buildOrderClause(sortBy = 'created_at', sortOrder = 'desc') {
        ...
        const validSortFields = ['id', 'created_at', 'updated_at', 'status', 'priority'];
    ...
    async list(pool, { filters = {}, page = 1, limit = 10, sortBy, sortOrder } = {}) {
        try {
        ...
        try {
            ...
            if (userIds.size > 0) {
                ...
                result.data = result.data.map(msg => ({
                    ...
                    assigned_to_user: msg.assigned_to ? userMap.get(msg.assigned_to) : null
                }));
            }
    ...
    async updateMessage(req, res) {
        try {
            ...
            if (currentMessage[0].assigned_to) {
                await notificationService.notify(currentMessage[0].assigned_to, notificationData);
            }
