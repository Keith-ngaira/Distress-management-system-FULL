...
export const distressMessages = {
    getAll: async (filters = {}) => {
        const response = await api.get('/api/distress-messages', { params: filters });
        const { data } = response;
        if (!data?.success || !data?.data) {
            throw new Error(data?.message || 'Invalid response format from server');
        }
    ...
    assign: async (id, assigneeId) => {
        const response = await api.post(`/api/distress-messages/${id}/assign`, { assigneeId });
        const { data } = response;
        if (!data?.success || !data?.data) {
            throw new Error(data?.message || 'Invalid response format from server');
        }
...
export const caseUpdates = {
    create: async (messageId, updateData) => {
        const response = await api.post(`/api/distress-messages/${messageId}/updates`, updateData);
        const { data } = response;
        if (!data?.success || !data?.data) {
            throw new Error(data?.message || 'Invalid response format from server');
        }
    ...
    getByMessageId: async (messageId) => {
        const response = await api.get(`/api/distress-messages/${messageId}/updates`);
        const { data } = response;
        if (!data?.success || !data?.data) {
            throw new Error(data?.message || 'Invalid response format from server');
        }
...
export const attachments = {
    upload: async (distressMessageId, file) => {
        ...
        const response = await api.post(`/api/attachments/${distressMessageId}/upload`, formData, {
            headers: {
